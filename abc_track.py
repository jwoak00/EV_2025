...
class ABCTrackNode(object):
    def __init__(self):
        ...
        self.lower_hls    = rospy.get_param('~lower_hls', [100,160,20])
        self.upper_hls    = rospy.get_param('~upper_hls', [172,200,220])

	    # self.lower_hls    = rospy.get_param('~lower_hls', [140,180,20])
        ...
        self.kp             = rospy.get_param('~kp', 0.7) #1.0
        self.lidar_speed    = rospy.get_param('~linear_speed', 0.4) #0.5

        # --- C구역 파라미터 ---
        self.goal_x_rel        = rospy.get_param('~goal_x_rel', 0.8)
        self.goal_y_rel        = rospy.get_param('~goal_y_rel', 0.8)
    ...
    def scan_callback(self, msg):
        ...
        li = int((math.radians(-100) - amin) / ainc)
        ri = int((math.radians(100) - amin) / ainc)
        def perp(center, idx):
            ds=[]
            for j in range(idx-tol, idx+tol+1):
                ...
                if not math.isinf(r):
                    ...
                    continue
            return ds
        lv = perp(math.radians(-100), li)
        rv = perp(math.radians(100), ri)
    ...
    def process_a(self):
        ...
        if (lx and len(lx)>0) or (rx and len(rx)>0):
            ...
            self.publish_control(0, throttle=0.7)
        try:
    ...
    # --- 유틸 함수 ---
    def publish_control(self, steer_deg, throttle=0.75):
